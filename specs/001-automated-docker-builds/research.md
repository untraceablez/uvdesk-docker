...
**Caveat + resolution (updated)**: Upstream's `Dockerfile` begins `FROM ubuntu:latest` (a floating base tag) and installs `mysql-server` + PHP from `ppa:ondrej/php`. This is fragile: when `ubuntu:latest` advanced to 26.04 "resolute", the ondrej/php PPA had no packages for it and the unmodified upstream build failed at `apt-get install php8.1`. **Resolution**: pin what `ubuntu:latest` resolves to at build time via `docker buildx --build-context ubuntu:latest=docker-image://ubuntu:24.04` (`UBUNTU_BASE_PIN`, default `ubuntu:24.04`) — reproducing the build against the Ubuntu upstream targeted **without editing upstream's Dockerfile or source**. Constitution v1.1.0 explicitly permits this build-environment pin under Principle I (reproducibility, not modification), which also makes builds effectively reproducible. Traceability is still preserved via the exact upstream version tag (FR-015) and build-time labels (upstream commit/tag + timestamp).
